        ...
        protected void btnUpdateCompanyDetails_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (objProfile.UpdateCompanyProfile(objCom, tempId) == true)
                {
                    ...
                    if (TextBox_addressID1.Text != "" && objProfile.UpdateCompanyAddress(objAdd1, int.Parse(TextBox_addressID1.Text)) == true)
                    {
                        if (TextBox_addressID2.Text != "") // update Address 2
                        {
                            if (objProfile.UpdateCompanyAddress(objAdd2, int.Parse(TextBox_addressID2.Text)) == true)
                                showErrorMessage("Updated Successfully", true);                          
                            else                            
                                showErrorMessage("Address 1 was not updated. Something gone wrong.", false);                            
                        }
                        if (TextBox_addressID2.Text == "")
                        {
                            if (objProfile.insertCompanyAddress(int.Parse(txtbox_CompanyID.Text), objAdd2) == true)
                                showErrorMessage("Updated Successfully", true);
                            else
                                showErrorMessage("Address 2 was not updated", false);
                        }
                        else                       
                            showErrorMessage("Updated Successfully", true);                      
                    }
                    ...
                    {
                        if (objProfile.insertCompanyAddress(int.Parse(txtbox_CompanyID.Text), objAdd1) == true)
                            showErrorMessage("Updated Successfully", true);
                        else
                            showErrorMessage("Address 2 was not updated", false);
